...
// Admin m2m-client provisioning handlers (INT-42, MA-6).
//
// Transport only: validates the request shape, delegates to the core
// ProvisionService, maps verdicts to HTTP responses. The plaintext secret is
// returned exactly once in the response body — it is never logged here (nor
// in the service).
...
func (a *API) PostLoginV1M2mClients(ctx context.Context, request PostLoginV1M2mClientsRequestObject) (PostLoginV1M2mClientsResponseObject, error) {
	...
	if a.provisioner == nil {
		logger.Error("m2m provisioning not configured")
